...
> 最后更新: 2026-06-16 (v2 — Reasonix 全功能完善)
...
**当前阶段:** 所有核心功能开发完成，可编译运行并通过设置页配置 API Key 后使用全部功能
...
- [x] STT / TTS API 接口定义
- [x] 国内 AI 模型预设列表
- [x] **AI 聊天真实 SSE 流式** — 逐 token 输出 + 打字光标动画 + 错误提示 + 重试
- [x] **完整对话历史** — sendMessage 包含全部历史消息
- [x] **API Key 安全存储** — EncryptedSharedPreferences (AES-256 GCM)，Room 中清零
- [x] **三套 UI 主题切换** — 像素伙伴/糖果乐园/星空童话，设置页切换，DataStore 持久化
- [x] **语音手动打断按钮** — 实时语音模式可随时打断 AI 播放
- [x] **小游戏 GameRegistry 工厂注册** — 统一注册表模式
- [x] **故事补充** — 新增 2 篇 SPACE 分类太空主题故事（共 12 篇）
- [x] **家长控制** — 每日使用时长限制 (5-120min) + 内容过滤开关 + 今日用量显示
...
- [ ] **儿童安全** — 内容审核中间层、敏感词过滤（目前已有关闭开关）
- [ ] **Google Play 上架准备** — 隐私政策、家长网关
- [ ] **游戏记录持久化** — 保存游戏得分和进度
- [ ] **AI 动态生成故事** — 不仅仅是播放预设 JSON 故事
- [ ] **唤醒词检测** — 语音模式增加唤醒词（如"Hey Buddy"）
- [ ] **STT/TTS URL 手动编辑** — 目前仅能通过预设切换，不能手动输入
- [ ] **自定义预设保存** — 用户可保存自己的 AI 模型配置预设
- [ ] **音量/灵敏度设置 UI** — 语音检测灵敏度调节
